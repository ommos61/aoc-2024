...
long counts[LINE_LENGTH];
void reset_counts(void) { for (unsigned i = 0; i < array_count(counts); i++) counts[i] = -1; }
long count_arrangements(char *design, unsigned index, unsigned depth) {
    long count = 0;

    if (depth == 0)  reset_counts();

    //if (debug) printf("  recurse(%s)\n", design + index);
    if (design[index] == 0) {
        if (debug) printf("    found new end setting counts[%u] to 1\n", index);
        if (counts[index] == -1) counts[index] = 1; else counts[index] += 1;
        count = 1;
    } else if (counts[index] != -1) {
        if (debug) printf("    using cached counts %ld\n", counts[index]);
        count = counts[index];
    } else {
        ...
        for (unsigned pi = 0; pi < pats.count; pi++) {
            ...
            if (ok) {
                //if (debug) printf("  used '%s', ", pats.pats[pi]);
                long num = count_arrangements(design, index + strlen(pats.pats[pi]), depth + 1);
                if (counts[index] == -1) counts[index] = num; else counts[index] += num;
                count += num;
            }
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    for (unsigned i = 0; i < design_count; i++) {
        ...
        long arrangements = count_arrangements(designs[des], 0, 0);
        if (debug) printf("additional arrangements: %ld\n", arrangements);
        if (arrangements != 0) {
            count += arrangements;
            if (0) {
    ...
    printf("There are %ld possible arrangements for all designs that can be made with the patterns\n", count);
